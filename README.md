...
AttributeGraph and a pluggable render backend. The cross-platform backend is **GTK4** (macOS,
Windows, Linux); **AppKit** and **Qt** backends are included to demonstrate the multi-backend seam.
...
| `HopUI` | SwiftUI-mirroring API (`View`, `@State`, `VStack`/`HStack`/`Text`/`Button`), view-graph evaluation, `DisplayList`, `Reconciler`, and the `RenderBackend` seam. |
| `CGTK4` | Hand-rolled C module over the GTK4 C ABI (resolved via `pkg-config gtk4`). |
| `HopGTK4` | GTK4 backend + `GtkApplication` runtime. |
| `HopAppKit` | AppKit backend (macOS only). |
| `CQt` / `HopQt` | C++ shim over Qt6 (pure-C surface) + Qt backend (macOS, via Homebrew Qt). |
| `HopDemo` | Shared `CounterView` used by every backend demo. |

## Prerequisites

- Swift 6.1+
- GTK4 (only for the GTK4 backend/demo):
...
- Qt6 (only for the Qt backend/demo, macOS): `brew install qt`
...
run them side by side to compare backends. Clicking a button or typing in the text field mutates
