...
swift build --package-path Demos/Showcase --product hop-demo-winui
if ($LASTEXITCODE -ne 0) { throw "swift build failed" }

# 3. Locate the build output + executable.
$binPath = (swift build --package-path Demos/Showcase --product hop-demo-winui --show-bin-path).Trim()
