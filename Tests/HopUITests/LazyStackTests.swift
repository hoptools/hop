...
@MainActor @Suite struct LazyStackTests {
    ...
    @Test func testLazyVStackMaterializesOnlyVisibleRows() throws {
        let toolkit = MockToolkit()  // 800×600 content area
        runHopApp(ScrollView {
            LazyVStack {
                ForEach(0 ..< 1000, id: \.self) { i in Text("Row \(i)") }
            }
        }, toolkit: toolkit, title: "t")
        toolkit.drainMainThread()  // converge the viewport-size + row-extent feedback

        let rows = liveRows(toolkit)
    ...
    @Test func testLazyVStackBelowAHeaderWindowsRelativeToItself() throws {
        let toolkit = MockToolkit()  // 800×600
        runHopApp(ScrollView {
            VStack(spacing: 0) {
        ...
        toolkit.drainMainThread()
        toolkit.drainMainThread()  // converge the row-extent + content-origin feedback
        #expect(liveRows(toolkit).contains("Row 0"))  // top of the list is visible below the header
