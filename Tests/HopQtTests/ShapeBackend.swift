...
// real QtBackend.drawShape. No display or QApplication needed (raster QPainter is self-contained).

import CQt
@testable import HopQt
import HopUI

enum ShapeBackend { static let name = "qt" }
...
final class ShapeCanvas {
    ...
    @MainActor func draw(_ spec: ShapeSpec, frameWidth: Double, frameHeight: Double, bleedX: Double = 0, bleedY: Double = 0) {
        QtBackend.drawShape(spec, frameWidth: frameWidth, frameHeight: frameHeight, bleedX: bleedX, bleedY: bleedY, painter: painter)
    }
