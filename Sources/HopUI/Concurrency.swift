...
// Swift Concurrency integration with the backend's run loop.
//
...
// but custom `TaskExecutor`s (SE-0417) ARE supported. So each backend provides a `HopLoopExecutor`
...
/// A custom `TaskExecutor` that runs jobs on a backend's native run loop. The backend supplies a
...
/// The backend's run-loop executor for Swift Concurrency, installed by the runtime when the app starts.
@MainActor
public enum HopConcurrency {
    public static var loopExecutor: HopLoopExecutor?
}

/// Starts async work that runs on the backend's run loop, so `await`/`Task.sleep` continuations resume
/// there (not on libdispatch's main queue, which GLib/Qt loops never drain). Mirrors a plain `Task {}`
/// for app code, but loop-correct on every backend. Mutate `@Observable` model state from inside; the
