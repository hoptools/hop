...
/// native widget shows the selected one), and the selection is held in a per-identity source so plain
/// `TabView { … }` works without an explicit binding.
public struct TabView<Content: View>: View, PrimitiveView {
    ...
    func makeNode(_ context: RenderContext) -> RenderNode {
        ...
        let selected = graph.read(selSource)

        let pages = evaluate(content, context.appending(0))
        let count = pages.count
        let current = count == 0 ? 0 : Swift.min(Swift.max(selected, 0), count - 1)
